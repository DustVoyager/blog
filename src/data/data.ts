...
export const data = {
  ...
  projects: [
    {
      ...
      problems: [
        "빌드 및 배포 복잡성: 프론트엔드와 백엔드가 하나의 프로젝트로 묶여 있어 빌드와 배포 과정이 복잡하고 시간이 많이 소요되었습니다.",
        "유연성 부족: 프론트엔드와 백엔드가 강하게 결합되어 있어, 각각의 기술 스택을 독립적으로 업데이트하거나 변경하기 어려웠습니다.",
      ],
      expectations: [
        "개발 속도 향상: 프론트엔드와 백엔드의 독립적 개발로 인해 개발 속도와 유연성 향상",
        "성능 개선: 프론트엔드와 백엔드의 분리로 서버 부하 감소 및 성능 최적화",
        "유지 보수 용이: 각기 독립적인 환경에서의 유지 보수가 용이해지고, 기술 스택의 자유로운 업데이트 가능",
      ],
      tasks: [
    ...
    },
    {
      ...
      challenges: [
        "데이터 통신 방식 변경: 기존 시스템은 서버에서 렌더링되는 구조였기 때문에 프론트엔드와 백엔드 간의 데이터 통신 방식을 API 기반으로 변경해야 했습니다. 이를 위해 RESTful API를 설계하고, 필요한 엔드포인트를 정의하여 데이터를 주고받는 작업이 필요했습니다.",
        "상태 관리: 사용자 인터페이스에서 다양한 상태를 관리하기 위해 Redux와 같은 상태 관리 라이브러리를 도입하여, 데이터의 일관성을 유지하고 효율적으로 상태를 관리하는 것이 중요했습니다.",
        "성능 최적화: 백엔드 렌더링에서 클라이언트 렌더링으로 전환하면서 발생할 수 있는 성능 저하를 최소화하기 위해 코드 스플리팅, Lazy Loading 등을 적용하여 사용자 경험을 개선했습니다.",
        "기존 시스템과의 호환성: 기존 백엔드 시스템과의 호환성을 유지하면서 새로운 프론트엔드 코드를 통합하는 과정에서 많은 테스트와 디버깅이 필요했습니다.",
      ],
      tasks: [
  ...
  tmis: [
    "육군에서 부사관으로 6년 동안 복무하고 중사로 제대하였습니다.",
    "웹 에이전시에서 마크업 개발자로 5년 있었습니다.",
    "매일 수영으로 하루를 시작하고 있습니다.",
    "책과 서점을 좋아합니다. 서점에서 풍기는 종이책의 냄새를 좋아하고 책장에 종류별로 진열하는 것을 좋아합니다.",
    "사람 만나는 것을 좋아합니다. 커피챗을 통해 다양한 사람들을 만나서 이야기를 나눠보고 인사이트를 넓히는 것을 좋아합니다.",
    ...
    "역사에 관심이 많습니다. 우연한 계기로 한국사 자격증을 취득하게 되면서 역사에 대한 공부를 많이 하였고 역사적인 인물들과 사건들에 대해 관심이 많습니다.",
